...
value. Both the value and the separator C<:> are optional. If a flag value
is not supplied it defaults to C<0E0> zero-but-true. Each flag has a short
name in addition to its canonical one. Flags supplied more than once have
their values aggregated in an arrayref (e.g. C<Depends>). Moreover some
flags always aggregate their values in an arrayref, even if there is only
one value supplied. See the known flag list on top of F<maint/dbic_todo> for
details.
