...
sub _resolve_collapse {
  ...
  if (
    ...
    my $parent_collapser_used = defined delete @{$idset}{keys %{$assumed_from_parent->{columns}}};
  ...
  $collapse_map->{-node_index} = ${ $node_idx_ref ||= \do { 1 } }++;  # this is *deliberately* not 0-based
...
sub _mk_row_parser {
  ...
  if ($args->{collapse}) {
    ...
    my $sequenced_top_node_id = join ('', map
...
### BEGIN STRING EVAL
  my ($rows_pos, $result_pos, $cur_row, $cur_row_ids, $is_new_res, @collapse_idx) = (0,0);
